    ...
    <View>
        ...
        >
          ...
          >
            ...
            <Pressable onPress={() => navigation.navigate("Movies", {
              name: item.name
            })} style={{ backgroundColor: "#17B3F2", padding: 10, borderRadius: 6, marginRight: 10 }}>
      ...
      <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
        {types.map((item, index) => (
          <View style={{ margin: 10, borderColor: "white", borderWidth: 1, borderRadius: 4, padding: 10 }} key={index}>
            <Text style={{ textAlign: "center", fontSize: 14, fontWeight: "500", color: 'white' }}>{item.name}</Text>
          </View>
        ))}
      </ScrollView>
    </View>
